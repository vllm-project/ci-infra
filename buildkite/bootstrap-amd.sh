...
if [[ -z "${VLLM_ROCM_BASE_IMAGE:-}" ]]; then
    VLLM_ROCM_BASE_IMAGE="rocm/vllm-dev:vllm-base-ubuntu24.04-py3.12-nightlies-device-all-rocm7.14.0a20260604-ad7125a43"
fi

if [[ -z "${VLLM_ROCM_CI_BASE_IMAGE:-}" ]]; then
    VLLM_ROCM_CI_BASE_IMAGE='rocm/vllm-dev:ci_base-rocm714-py312-$BUILDKITE_COMMIT'
fi

if [[ -z "${VLLM_ROCM_CI_BASE_IMAGE_TAG_BASE:-}" ]]; then
    VLLM_ROCM_CI_BASE_IMAGE_TAG_BASE="rocm/vllm-dev:ci_base-rocm714-py312"
fi
