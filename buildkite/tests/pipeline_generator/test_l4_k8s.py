...
def test_l4_step_keeps_explicit_retry():
    ...
    # agent-loss retries that main adds to every step.
...
def test_non_l4_steps_keep_existing_routing(device, num_devices, queue):
    ...
    # Every step gets the repo-wide agent-loss retries; non-l4 steps get
    # nothing beyond them (in particular not K8S_RETRY).
    assert command_step.retry == {
        "automatic": [
            buildkite_step.EXIT_STATUS_NEGATIVE_ONE_RETRY,
            buildkite_step.AGENT_STOP_RETRY,
        ]
    }
