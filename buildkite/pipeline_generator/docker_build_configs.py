  ...
  exit 0
    ...
    def _get_docker_build_command(self) -> str:
        ...
        if self.use_fastcheck_formatting:
            # Fastcheck uses folded scalar (>) which adds double spaces except
            # for last 3 args before tag
...
def create_cuda_build_config(
    ...
    build_args = {"max_jobs": "16", "buildkite_commit": commit}
...
class AMDBuildConfig:
    ...
    def to_buildkite_step(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "commands": [self.get_build_command(), f"docker push {image_tag}"],
        }


def create_main_cuda_build(
    commit: str,
    image_tag: str,
    queue: str,
    branch: str,
...
def create_torch_nightly_build(
    ...
    return create_cuda_build_config(
        ...
        key=BuildStepKeys.TORCH_NIGHTLY_IMAGE,
        image_tag=image_tag,
        queue=queue,
        commit=commit,
        depends_on=depends_on,
