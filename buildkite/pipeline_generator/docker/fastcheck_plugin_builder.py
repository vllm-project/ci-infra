...
def build_fastcheck_environment(test_step, config) -> DockerEnvironment:
...
def build_fastcheck_docker_command(test_step, config) -> str:
...
def build_fastcheck_docker_plugin(test_step, container_image: str, config) -> Dict:
    ...
    mount_agent = (
        test_step.label == TestLabels.BENCHMARKS or 
        test_step.mount_buildkite_agent
    )
...
def build_fastcheck_a100_kubernetes_plugin(test_step, container_image: str, config) -> Dict:
