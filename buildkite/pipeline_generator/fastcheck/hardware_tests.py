...
from ..hardware_test_configs import GH200_CONFIG, get_tpu_v0_notification_config
from ..utils.constants import BlockLabels, HardwareLabels, Scripts
...
def _add_tpu_v1_tests(
        all_hw_tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Add TPU V1 tests for fastcheck."""
    steps = []

    for test in all_hw_tests:
        if isinstance(test, dict) and test.get("label", "") == "TPU V1 Test":
            steps.append({"block": "Run TPU V1 Test",
                         "key": "block-tpu-v1", "depends_on": None})
            test_copy = test.copy()
...
def _add_gh200_test() -> List[Dict[str, Any]]:
    ...
    return [{"block": "Run GH200 Test", "depends_on": None,
             "key": "block-gh200"}, gh200_step]


def _add_intel_tests(
        all_hw_tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for test in all_hw_tests:
        if isinstance(test, dict):
            ...
            if "Intel CPU Test" in label:
                steps.append({"block": "Run Intel CPU test",
                              "key": "block-intel-cpu", "depends_on": None})
                test_copy = test.copy()
                test_copy["depends_on"] = "block-intel-cpu"
                steps.append(test_copy)
            elif "Intel GPU Test" in label:
                steps.append({"block": "Run Intel GPU test",
                              "key": "block-intel-gpu", "depends_on": None})
                test_copy = test.copy()
...
def get_tpu_v0_tests() -> List[Dict[str, Any]]:
    ...
    return _add_tpu_v0_tests()


def get_tpu_v1_tests(
        all_hw_tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Get TPU V1 tests."""
    return _add_tpu_v1_tests(all_hw_tests)


def get_gh200_test() -> List[Dict[str, Any]]:
    """Get GH200 test."""
    return _add_gh200_test()


def get_intel_tests(
        all_hw_tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Get Intel CPU and GPU tests."""
    return _add_intel_tests(all_hw_tests)


def generate_fastcheck_hardware_tests(
        branch: str, nightly: bool) -> List[Dict[str, Any]]:
    """
    Generate hardware tests for fastcheck mode.

    DEPRECATED: Use individual get_* functions for clearer control.
