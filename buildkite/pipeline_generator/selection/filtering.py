...
def should_run_step(step, config) -> bool:
...
def get_intelligent_test_targets(test_step, changed_tests: List[str]) -> List[str]:
    ...
    for target in matched_targets:
        for covered_path in covered_paths:
            # Check if target starts with this covered path
            if target.startswith(covered_path):
                # Ensure it's a proper directory match
                if len(target) == len(covered_path) or target[len(covered_path)] == "/":
...
def extract_covered_test_paths(commands) -> List[str]:
    ...
    for cmd in commands:
        ...
        for part in cmd_parts:
            if part == "pytest":
                in_pytest = True
            elif in_pytest and not part.startswith("-") and "/" in part:
                # This looks like a test path (contains / and doesn't start with -)
                # Skip specific tests (contain ::)
                if "::" not in part:
                    if part.endswith(".py"):
                        # It's a file - add the file and its directory
...
def extract_pytest_markers(commands) -> str:
    ...
    for cmd in commands:
        if "pytest " in cmd and " -m " in cmd:
            ...
            if len(parts) > 1:
                after_m = parts[1]
                # Handle quoted markers
                if after_m.startswith("'"):
                    marker_parts = after_m[1:].split("'")
                    if marker_parts:
                        return f" -m '{marker_parts[0]}'"
