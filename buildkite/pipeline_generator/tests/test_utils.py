...
class TestAgentQueue:
    ...
    def test_get_agent_queue(
            self,
            no_gpu: bool,
            gpu_type: str,
            num_gpus: int,
            label: str,
            expected_result: AgentQueue):
        """Test agent queue selection based on requirements."""
        assert get_agent_queue(
            no_gpu,
            gpu_type,
            num_gpus,
...
class TestFullTestCommand:
    ...
    @pytest.mark.parametrize(("test_commands",
                              "step_working_dir",
                              "expected_result"),
                             [(["echo 'hello'"],
                               None,
    ...
    def test_get_full_test_command(
            self,
            test_commands: List[str],
            step_working_dir: str,
            expected_result: str):
        """Test full command generation with working directory."""
        assert get_full_test_command(
...
class TestMultiNodeTestCommand:
    ...
    def test_get_multi_node_test_command(self):
        ...
        result = get_multi_node_test_command(
            test_commands, working_dir, num_nodes, num_gpus, docker_image_path
        )
    ...
    def test_get_multi_node_test_command_single_node_command(self):
        ...
        result = get_multi_node_test_command(
            test_commands, "/tests", 2, 2, "test:latest")
