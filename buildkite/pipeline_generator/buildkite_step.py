...
# Coverage-based test selection: load a mapping of {source_file: [step_keys]}
# generated by nightly CI runs. Used as a fallback when source_file_dependencies
# doesn't match, catching transitive dependencies that manual lists miss.
_coverage_map: Optional[Dict[str, List[str]]] = None
_COVERAGE_MAP_URL = "https://vllm-ci.s3.us-west-2.amazonaws.com/coverage/coverage_map.json"


def _load_coverage_map() -> Dict[str, List[str]]:
    global _coverage_map
    if _coverage_map is not None:
        return _coverage_map
    try:
        with _urllib_request.urlopen(_COVERAGE_MAP_URL, timeout=5) as resp:
            _coverage_map = _json_module.loads(resp.read())
            print(f"[coverage-map] Loaded {len(_coverage_map)} source files from S3")
    except Exception as e:
        print(f"[coverage-map] Failed to load from S3: {e}. Falling back to source_file_dependencies only.")
        _coverage_map = {}
    return _coverage_map
...
def _step_should_run(step: Step, list_file_diff: List[str]) -> bool:
    ...
    if coverage_map:
        for diff_file in list_file_diff:
            if step_key in coverage_map.get(diff_file, []):
                matched_by_coverage = True
                break

    if matched_by_coverage and not matched_by_yaml:
