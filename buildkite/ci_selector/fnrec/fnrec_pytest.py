...
to $FNREC_DIR/pytest.<pid>.txt. The collection line goes out on its own and
...
Loaded by a `pytest11` entry point that fnrec/ci_setup.sh installs next to
fnrec.py, so every pytest the image's interpreter starts loads it and nothing
else does. Inert without FNREC_DIR and in an xdist worker (the controller
...
def pytest_configure(config):
    try:
        directory = os.environ.get("FNREC_DIR")
