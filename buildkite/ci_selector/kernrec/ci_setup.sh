...
# Write $KERNREC_DIR/kernrec.json: what the table builder needs to file this
# job under its step and to judge the row. $1 is the exit status, or `null`
# while the step is still running. Written once at setup, so a job killed
# before its trap runs still has a step key and an unknown status (which the
# builder treats as not passed), and rewritten with the real status on exit.
kernrec_write_sidecar() {
  ...
  chmod 0666 "$KERNREC_DIR/kernrec.json" 2>/dev/null || true
}

# Runs when the step's shell exits (not after a SIGKILL). Records the exit
# status, then opens permissions so the agent can clean the checkout,
