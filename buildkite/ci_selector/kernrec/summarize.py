...
def read_dir(d: Path) -> dict:
    ...
    for f in sorted(d.glob("kern.*.txt")):
        p = {"file": f.name, "pid": None, "exe": None, "unique": 0,
             "records": None, "dropped": 0, "clean_exit": False}
...
def main() -> int:
    ...
    checks["min_procs"] = (len(procs) >= a.min_procs,
    ...
    print(f"--- {a.label or a.dir} : {'PASS' if ok else 'FAIL'}"
    ...
    for p in procs:
        ...
        print(f"      pid={p['pid']} exe={exe} unique={p['unique']} "
    ...
    if a.json:
        a.json.write_text(json.dumps({
            "label": a.label, "ok": ok, "pytest_rc": a.pytest_rc,
            "seconds": a.seconds, "checks": checks, "procs": procs,
            "names": names, "demangled": dm}, indent=1))
