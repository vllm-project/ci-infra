...
config:
  ...
  # How many steps the controller will hold at once. The chart's default is 25,
  # and a single nightly reaches it: the JAX suite fans out into far more
  # four-chip steps than eight chips can admit, so the cap binds while most of
  # those pods are doing nothing but waiting for hardware.
  #
  # Waiting in a pod is not the wasteful half of that. Kueue is the queue, and a
  # step that cannot get chips inside tpu_queue_max_seconds says so and names the
  # fleet as the cause, with job-active-deadline-seconds derived to sit above it
  # so the deadline never fires first. What the cap costs is the opposite: work
